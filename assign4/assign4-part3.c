...
void createPhilosophers(int nthreads)
{
    ...
    // Initialize CV and mutex to access it
    pthread_mutex_init(&index_lock, NULL);
    pthread_cond_init(&next_cv, NULL);
...
void *philosopherThread(void *arg)
{
    ...
    // Stop thinking, then get lock on the index_lock
    pthread_mutex_lock(&index_lock);
    // Loop while the it is not this loop's turn
    while (next_index != number) {
        pthread_cond_wait(&next_cv, &index_lock);
    }

    // Once it is our turn, pick up the chopsticks
    ...
    // Done eating, now put down chopsticks
